...
const UserListings = () => {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    // Simulate API call to fetch user's car listings
    setTimeout(() => {
      // Filter cars that belong to the current user
      const userCars = mockCars.filter(car => car.ownerId === currentUser.id);
      setCars(userCars);
      
      // Count test drive requests for each car
      const requestCounts: Record<string, number> = {};
      mockTestDriveRequests.forEach(request => {
        const car = mockCars.find(c => c.id === request.carId);
  ...
  const handleDeleteCar = () => {
    ...
    const index = mockCars.findIndex(c => c.id === selectedCar.id);
    if (index !== -1) {
      ...
      <div className="min-h-screen flex flex-col">
        ...
        <div className="container mx-auto px-4 py-8 flex-grow">
          <div className="flex justify-between items-center mb-6">
            ...
            <Button onClick={() => navigate('/user/list-car')}>
